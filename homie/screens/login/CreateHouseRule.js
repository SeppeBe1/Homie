...
const CreateHouseRule = ({navigation}) => {
  ...
  const [editDescriptionId, setEditRuleId] = useState(null);
            ...
            fetch('http://localhost:3000/api/v1/houseRules', {
              ...
              .then(data => {
                  if(data.status == "failed"){
                    ...
                    console.log(data.status);
                    setDescriptionsList((prevDescriptionsList) => [...prevDescriptionsList, newDescription]);
                    setDescription('');
                  }
    ...
    const deleteDescription = async (descriptionId, descriptionText) => {
      ...
      fetch(`http://localhost:3000/api/v1/houseRules/${descriptionText}/${houseId}`, {
        method: 'DELETE',
        headers: {
            'Authorization': `Bearer ${token}`,
        },
  ...
  return (
    <View style={styles.container}>
        ...
        <Text style={styles.h3}> 3. Are there certain house rules that apply to your home?</Text>
      ...
      <View style={styles.input}>
        ...
        <TouchableOpacity onPress={createDescription}>
          <Text>{editDescriptionId ?  <Image source={update} style={styles.inputEdit}/> : <Image source={plus} style={styles.inputEdit}/>}</Text>
        </TouchableOpacity>
      </View>
